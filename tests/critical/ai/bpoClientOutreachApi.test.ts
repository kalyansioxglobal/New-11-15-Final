...
import handler from "@/pages/api/ai/bpo-client-draft";
...
jest.mock("@/lib/ai/bpoClientOutreachAssistant", () => ({
  generateBpoClientOutreachDraft: jest.fn().mockResolvedValue("draft text"),
}));
...
describe("POST /api/ai/bpo-client-draft", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it("returns 503 when AI flags are disabled", async () => {
    ...
    (requireUser as jest.Mock).mockResolvedValue({ id: 1, role: "BPO_MANAGER" });

    const { req, res } = createMockReqRes({
      draftType: "cold_outreach",
