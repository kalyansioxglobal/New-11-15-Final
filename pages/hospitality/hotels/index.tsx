...
export default function HotelsListPage() {
  ...
  const [ventures, setVentures] = useState<{ id: number; name: string; type: string }[]>([]);
  ...
  useEffect(() => {
    fetch(`/api/ventures?types=HOSPITALITY&includeTest=${testMode}`)
      .then(r => r.json())
      .then(d => {
        setVentures(d as { id: number; name: string; type: string }[]);
      })
      .catch(() => {});
  }, [testMode]);
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600">
            {hotels.length} hotels | {totalRooms} total rooms
          </p>
        </div>
                ...
                hotels.map(h => (
                  <tr key={h.id} className="border-t hover:bg-gray-50">
                    <td className="px-4 py-2">
                      ...
                      {h.code && (
                        <span className="text-xs text-gray-500 ml-2">({h.code})</span>
                      )}
                    </td>
                    <td className="px-4 py-2 text-gray-600">{h.brand || '—'}</td>
                    <td className="px-4 py-2 text-gray-600">
                      {[h.city, h.state, h.country].filter(Boolean).join(', ') || '—'}
                    </td>
                    <td className="px-4 py-2 text-gray-600">
                      <Link href={`/ventures/${h.venture.id}`} className="hover:underline">
                        {h.venture.name}
                      </Link>
                    </td>
                    <td className="px-4 py-2 text-right">
                      {h.rooms != null ? h.rooms : '—'}
                    </td>
                    <td className="px-4 py-2 text-center">
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${STATUS_COLORS[h.status] || 'bg-gray-100'}`}>
                        {h.status}
                      </span>
                    </td>
                  </tr>
