...
function VerticalBadge({ vertical }: { vertical: string | null }) {
  ...
  return (
    <span className="inline-block mt-1 text-xs px-2 py-0.5 bg-slate-700/50 text-slate-300 rounded">
      {vertical}
    </span>
  );
}
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-100">Shippers (Accounts)</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <input
          ...
          className="flex-1 sm:max-w-xs px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
        />
        <select
          value={ventureFilter}
          onChange={(e) => setVentureFilter(e.target.value)}
          className="px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
        >
      ...
      {loading ? (
        ...
        /* Empty State */
        <div className="text-center py-16 border border-slate-700 rounded-xl bg-slate-800/50">
          <div className="text-slate-500 text-4xl mb-4">👥</div>
          <h3 className="text-slate-200 font-semibold text-lg mb-2">No Shipper Accounts Found</h3>
        ...
        <>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="text-right shrink-0">
                    <div className="text-xl font-bold text-slate-100">{c._count?.loads || 0}</div>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    {c.phone && <div className="text-slate-400 text-xs">{c.phone}</div>}
                  </div>
                  <div>
                    <div className="text-xs text-slate-500 uppercase tracking-wide mb-1">Venture</div>
                    <div className="text-slate-300">{c.venture?.name || "-"}</div>
                  </div>
                </div>

                <div className="grid grid-cols-3 gap-3 pt-3 border-t border-slate-700 text-sm">
                  <div>
                    <div className="text-xs text-slate-500 uppercase tracking-wide mb-1">Sales Rep</div>
                    <AssignmentBadge name={c.salesRep?.name} />
                  </div>
                  <div>
                    <div className="text-xs text-slate-500 uppercase tracking-wide mb-1">CSR</div>
                    <AssignmentBadge name={c.csr?.name} />
                  </div>
                  <div>
                    <div className="text-xs text-slate-500 uppercase tracking-wide mb-1">Dispatcher</div>
                  ...
                  {filteredCustomers.map((c) => (
                    <tr key={c.id} className="hover:bg-slate-700/30 transition-colors">
                      ...
                      <td className="px-5 py-4 text-sm text-right">
                        <span className="font-semibold text-slate-100">{c._count?.loads || 0}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2">
            <p className="text-sm text-slate-400">
              Showing <span className="font-medium text-slate-200">{filteredCustomers.length}</span> of{" "}
              <span className="font-medium text-slate-200">{total}</span> accounts
            </p>
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-4 py-2 rounded-lg border border-slate-600 bg-slate-800 text-slate-300 text-sm font-medium hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
                Previous
              </button>
